...
export default {
  ...
  methods: {
    ...
    editRolesInfo() {
      this.$refs.editRolesRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.put(
          'roles/' + this.editFromRoles.roleId,

          {
            roleName: this.editFromRoles.roleName,
            roleDesc: this.editFromRoles.roleDesc
          }
        )
        if (res.meta.status !== 200) {
    ...
    getLeefkeys(node, arr) {
      //如果当前noed节点不包含children属性，则是三级节点
      if (!node.children) {
